...
export const api = {
  ...
  admin: {
    ...
    publishNewsletter: () => apiFetch<{ id: string; slug: string; title: string; publishedAt: string }>('/admin/newsletter/publish', { method: 'POST', body: JSON.stringify({}) }),
    sendNewsletter: () => apiFetch<{ sent: number; recipients: number; batches: number; publicInsight: { id: string; slug: string } | null }>('/admin/newsletter/send', { method: 'POST', body: JSON.stringify({}) }),
    newsletterImages: () => apiFetch<Array<Record<string, unknown>>>('/admin/newsletter/images'),
    uploadNewsletterImage: (body: FormData) => apiFetch<Record<string, unknown>>('/admin/newsletter/images', { method: 'POST', body }),
