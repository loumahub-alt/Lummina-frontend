...
const Header = ({ eyebrow, title, description }: { eyebrow: string; title: string; description?: string }) => (
  <div className="mb-8">
    ...
    {description && <p className="mt-3 max-w-2xl leading-7 text-ink/60">{description}</p>}
  </div>
);
...
export const LiveNewsletterPage = () => {
  ...
  const sendNewsletter = async () => {
    ...
    try {
      ...
      setMessage('Newsletter queued for ' + result.sent + ' subscribed recipient' + (result.sent === 1 ? '' : 's') + (result.publicInsight ? ' and added to public Insights.' : '.'));
    } catch (reason) {
  ...
  const publishToInsights = async () => {
    if (templateDirty) {
      setError('Save the newsletter template before publishing it to Insights.');
      return;
    }
    if (!template.html.trim() || !template.subject.trim()) {
      setError('Save a newsletter subject and HTML template before publishing it to Insights.');
      return;
    }
    if (!window.confirm('Publish this saved newsletter to the public Insights page?')) return;
    setPublishing(true);
    setError('');
    setMessage('');
    try {
      await api.admin.publishNewsletter();
      setMessage('Newsletter published to Insights. Visitors can now find it under Newsletters.');
    } catch (reason) {
      setError(apiError(reason, 'Unable to publish the newsletter to Insights.'));
    } finally {
      setPublishing(false);
    }
  };
